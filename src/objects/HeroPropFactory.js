...
function buildRocketStand(g) {
  ...
  nose.position.y = 3.7;
  g.add(nose); meshes.push(nose);
  // Engine glow
  const engine = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.35, 0.3, 12), mat(0xff8800, 0xff4400, 0.8));
  engine.position.y = 0.35;
  g.add(engine); meshes.push(engine);
  return meshes;
}
...
function buildSatelliteDish(g) {
  ...
  pole.position.y = 1;
  g.add(pole); meshes.push(pole);
  // Dish (flattened sphere)
  const dish = new THREE.Mesh(new THREE.SphereGeometry(1, 16, 8, 0, Math.PI * 2, 0, Math.PI / 3), mat(0xaaaacc));
  dish.position.y = 2.2;
  dish.rotation.x = Math.PI;
  g.add(dish); meshes.push(dish);
  return meshes;
}

function buildAsteroidCluster(g) {
  const meshes = [];
  const positions = [[0, 0.5, 0], [-0.8, 0.3, 0.5], [0.6, 0.4, -0.4], [0.2, 0.8, 0.3]];
  for (const [x, y, z] of positions) {
    ...
    g.add(asteroid); meshes.push(asteroid);
  }
  return meshes;
}
...
function buildWormholeRing(g) {
  const meshes = [];
  const ring = new THREE.Mesh(new THREE.TorusGeometry(1.5, 0.15, 8, 32), mat(0x8800ff, 0x8800ff, 0.6));
  ring.position.y = 1.5;
  ring.rotation.x = Math.PI / 6;
  g.add(ring); meshes.push(ring);
  // Inner glow disc
  const disc = new THREE.Mesh(
    new THREE.CircleGeometry(1.3, 32),
    new THREE.MeshStandardMaterial({ color: 0x6600cc, emissive: 0x6600cc, emissiveIntensity: 0.4, transparent: true, opacity: 0.3, side: THREE.DoubleSide })
  );
  disc.position.y = 1.5;
  disc.rotation.x = -Math.PI / 2 + Math.PI / 6;
  g.add(disc); meshes.push(disc);
  return meshes;
}
...
function buildEnergyCollector(g) {
  ...
  g.add(tower); meshes.push(tower);
  // Solar panels (3 angled discs)
  for (let i = 0; i < 3; i++) {
    const panel = new THREE.Mesh(new THREE.BoxGeometry(1.5, 0.05, 0.8), mat(0x2244aa, 0x1122aa, 0.3));
...
function buildBlackHoleCore(g) {
  ...
  const core = new THREE.Mesh(new THREE.SphereGeometry(0.8, 16, 16), new THREE.MeshStandardMaterial({ color: 0x000000, roughness: 0, metalness: 1 }));
  core.position.y = 1.5;
  g.add(core); meshes.push(core);
  // Accretion disc
  const disc = new THREE.Mesh(new THREE.TorusGeometry(1.5, 0.2, 6, 32), mat(0xff6600, 0xff4400, 0.6));
  disc.position.y = 1.5;
...
function buildStationReactor(g) {
  const meshes = [];
  // Core cylinder
  const core = new THREE.Mesh(new THREE.CylinderGeometry(0.6, 0.6, 3, 16), mat(0x446688, 0x2244aa, 0.2));
  core.position.y = 1.5;
  g.add(core); meshes.push(core);
  // Top ring
  const ring = new THREE.Mesh(new THREE.TorusGeometry(0.9, 0.1, 8, 24), mat(0x88aacc, 0x44aaff, 0.4));
  ring.position.y = 2.8;
  g.add(ring); meshes.push(ring);
  // Bottom ring
  const ring2 = ring.clone();
  ring2.position.y = 0.2;
  g.add(ring2); meshes.push(ring2);
  // Glow sphere at center
  const glow = new THREE.Mesh(new THREE.SphereGeometry(0.4, 12, 12), mat(0x44ddff, 0x44ddff, 0.8));
  glow.position.y = 1.5;
  g.add(glow); meshes.push(glow);
  return meshes;
}
