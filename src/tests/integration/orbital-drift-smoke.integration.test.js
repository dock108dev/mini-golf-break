/**
 * Smoke tests for all 9 Orbital Drift hole configs.
 * ISSUE-019
 *
...
const EXPECTED_MECHANICS = [
  { index: 0, description: '1. Launch Bay', types: ['moving_sweeper'] },
  ...
  { index: 3, description: '4. Asteroid Belt Bounce', types: ['ricochet_bumpers', 'elevated_green'] },
  { index: 4, description: '5. Wormhole Transfer', types: ['portal_gate'] },
  { index: 5, description: '6. Solar Flare Run', types: ['timed_hazard', 'timed_hazard', 'timed_hazard'] },
  ...
  { index: 8, description: '9. Station Core Finale', types: ['split_route', 'moving_sweeper', 'boost_strip', 'elevated_green'] },
];
...
describe('Orbital Drift hole configs smoke tests (ISSUE-019)', () => {
  ...
  describe('config loading', () => {
    it('loads exactly 9 hole configs', () => {
      expect(configs).toHaveLength(9);
    });

    it('each config has sequential indices 0-8', () => {
